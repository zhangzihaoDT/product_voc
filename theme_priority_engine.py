...
def _jtbd_id(l1: str, l2: str) -> str:
    base = f"{_to_str(l1)}|{_to_str(l2)}"
    return hashlib.sha256(base.encode("utf-8")).hexdigest()[:16]
...
def build_jtbd_fact(issue_fact_refined: pd.DataFrame) -> pd.DataFrame:
    df = issue_fact_refined.copy()
    if "is_voc" in df.columns:
        is_voc = df["is_voc"].astype(str).str.lower().isin(["true", "1", "yes"])
        df = df[is_voc].copy()

    df["jtbd_l1"] = df.get("jtbd_l1", "").map(_to_str)
    df["scenario_l2"] = df.get("scenario_l2", "").map(_to_str)
    df.loc[df["scenario_l2"] == "", "scenario_l2"] = "其他"
    df.loc[df["jtbd_l1"] == "", "jtbd_l1"] = "其他"
    ...
    grouped = []
    for (l1, l2), sub in df.groupby(["jtbd_l1", "scenario_l2"], dropna=False):
        l1s = _to_str(l1) or "其他"
        l2s = _to_str(l2) or "其他"
        issue_count = int(len(sub))
        app_count = int(sub["channel"].astype(str).str.contains("APP", case=False, na=False).sum()) if "channel" in sub.columns else 0
    ...
    jtbd = pd.DataFrame(grouped)
    if jtbd.empty:
        return jtbd.reindex(columns=JTBD_FACT_COLUMNS)

    max_raw = float(jtbd["priority_score_raw"].max()) if not jtbd["priority_score_raw"].isna().all() else 0.0
    if max_raw > 0:
        jtbd["priority_score"] = (jtbd["priority_score_raw"] / max_raw * 100.0).round(2)
    else:
        jtbd["priority_score"] = 0.0

    jtbd = jtbd.sort_values(["priority_score", "issue_count"], ascending=[False, False])
    jtbd = jtbd.drop(columns=["priority_score_raw"], errors="ignore").reindex(columns=JTBD_FACT_COLUMNS)
    return jtbd
...
def main() -> int:
    ...
    parser.add_argument("--output-jtbd", default="jtbd_fact.csv")
    ...
    jtbd = build_jtbd_fact(issue)
    jtbd.to_csv(args.output_jtbd, index=False, encoding="utf-8-sig")

    other_count = export_need_theme_other(issue, Path(args.output_other))
    print(f"Wrote {len(theme)} rows to {args.output_theme}")
    print(f"Wrote {len(jtbd)} rows to {args.output_jtbd}")
